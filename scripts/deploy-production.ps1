...
function Get-PowerShellCommand {
  ...
  throw "PowerShell executable not found for nested production script execution."
}
...
function Get-SkippedSafetyGates {
  $skipped = @()
  if ($SkipSecurityAudit) { $skipped += "security audit" }
  if ($SkipBackup) { $skipped += "backup" }
  if ($SkipMigratePreflight) { $skipped += "migration preflight" }
  if ($SkipSmoke) { $skipped += "smoke test" }
  if ($SkipLogCheck) { $skipped += "log scan" }
  if ($NoBuild) { $skipped += "image build" }
  return $skipped
}
...
$skippedSafetyGates = @(Get-SkippedSafetyGates)
if ($skippedSafetyGates.Count -gt 0) {
  if (-not $ConfirmSkipGates) {
    throw "Skipping safety gates requires explicit confirmation. Re-run with -ConfirmSkipGates after approving skipped gate(s): $($skippedSafetyGates -join ', ')."
  }
  Write-Warning "Skipping approved safety gate(s): $($skippedSafetyGates -join ', ')"
}

$gitStatus = @(Get-GitStatusPorcelain)
if ($gitStatus.Count -gt 0) {
  $sample = ($gitStatus | Select-Object -First 5) -join "; "
  if ($DryRun) {
    Write-Warning "Git worktree has uncommitted changes. Real deploy requires -AllowDirtyWorktree. Sample: $sample"
  } elseif (-not $AllowDirtyWorktree) {
    throw "Git worktree has uncommitted changes. Commit/stash changes or re-run with -AllowDirtyWorktree after approving this deployment. Sample: $sample"
  } else {
    Write-Warning "Deploying from dirty git worktree due to -AllowDirtyWorktree. Sample: $sample"
  }
}

$composeFiles = @("docker-compose.yml", "docker-compose.prod.yml")
if ($WithProxy) {
  $composeFiles += "docker-compose.proxy.yml"
}
if ($WithObservability) {
  $composeFiles += "docker-compose.observability.yml"
}
...
Invoke-Step "Validate Docker Compose configuration" {
  Invoke-CommandLine -Command (@("docker") + $composeArgs + @("config", "--quiet")) -RunInDryRun
}
...
if (-not $SkipSmoke) {
  Invoke-Step "Validate smoke credentials" {
    if (-not (Test-SmokeCredentialsConfigured)) {
      throw "Smoke test is enabled but no dedicated smoke credential is configured. Set SMOKE_ADMIN_EMAIL+SMOKE_ADMIN_PASSWORD or SMOKE_ADMIN_ACCESS_TOKEN, or re-run with -SkipSmoke only for an explicitly approved deploy."
    }
    Write-Host "Smoke credentials configured."
  }
}

if (-not $SkipMigratePreflight) {
  Invoke-Step "Run migration preflight" {
    $preflightArgs = @("scripts/production-migration-preflight.js", "--env-file", $EnvFile)
    foreach ($file in $composeFiles) {
      $preflightArgs += @("--compose-file", $file)
    }
    if ($WithObservability) {
      $preflightArgs += @("--profile", "observability")
    }
    if ($WithProxy) {
      $preflightArgs += @("--profile", "proxy")
    }
    Invoke-CommandLine -Command (@("node") + $preflightArgs)
  }
}

if (-not $SkipBackup) {
  Invoke-Step "Create pre-deploy backup" {
    $powerShellCommand = Get-PowerShellCommand
    $backupCommand = if ($powerShellCommand -eq "powershell") {
      @($powerShellCommand, "-ExecutionPolicy", "Bypass", "-File", "scripts/backup-production.ps1", "-EnvFile", $EnvFile)
    } else {
      @($powerShellCommand, "-File", "scripts/backup-production.ps1", "-EnvFile", $EnvFile)
    }
    Invoke-CommandLine $backupCommand
  }
}
...
Invoke-Step "Deploy Docker stack" {
  Invoke-CommandLine -Command (@("docker") + $composeArgs + $upArgs)
}
...
Write-Host ""
Write-Host "Production deployment workflow completed."
